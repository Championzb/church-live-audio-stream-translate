...
- Utility actions are distributed for faster access: `Test Audio File` and `Export Transcript` are icon buttons on the transcription panel header, while `Reset Session (F4)` stays in the hotkey strip and translation-mode live bar.
