...
const labelMainGroqApiKeyEl = document.getElementById('labelMainGroqApiKey');
const labelMainSttProviderEl = document.getElementById('labelMainSttProvider');
...
const mainGroqApiKeyInput = document.getElementById('mainGroqApiKeyInput');
const mainSttProviderSelect = document.getElementById('mainSttProvider');
...
const copyMainGroqApiKeyButton = document.getElementById('copyMainGroqApiKey');
...
const settingsNavAppearanceButton = document.getElementById('settingsNavAppearance');
const settingsNavRuntimeButton = document.getElementById('settingsNavRuntime');
const settingsNavLanguageAidsButton = document.getElementById('settingsNavLanguageAids');
...
const settingsControlsEl = document.getElementById('settingsControls');
...
const labelGroqApiKeyEl = document.getElementById('labelGroqApiKey');
const labelSttProviderEl = document.getElementById('labelSttProvider');
...
const UI_TEXT = {
    en: {
        ...
        'cost.realNeedsProject': 'Real cost requires Project ID in Settings.',
    ...
    'zh-hans': {
        ...
        'cost.realNeedsProject': '真实费用需要在设置中填写 Project ID。',
        'cost.project': 'Project：{projectId}',
...
function saveSttProvider(rawProvider) {
    const provider = normalizeSttProvider(rawProvider);
...
function updateCostSummary() {
    ...
    if (projectId) {
        if (cachedRealCostProjectId === projectId && !cachedRealCostError) {
            ...
            maskedApiKeyEl.title = `${t('cost.realUnavailable', {
                reason: cachedRealCostError
            })}\n${groqRealCostLine}\n${t('cost.project', { projectId })}`;
        }
        else {
            maskedApiKeyEl.title = `${t('cost.realLoading')}\n${t('cost.project', {
                projectId
            })}\n${groqRealCostLine}`;
        }
    ...
    maskedApiKeyEl.title = `${t('cost.realNeedsProject')}\n${groqRealCostLine}`;
}
...
function updateObservabilityStrip() {
    ...
    const tierKey = (state) => (state === 'danger' ? 'high' : state === 'warn' ? 'elevated' : 'good');
...
function initSettingsContextNav() {
    ...
    if (!settingsControlsEl || !sectionButtons.length)
        return;
    sectionButtons.forEach((button) => {
    ...
    sectionButtons.forEach((button) => {
        ...
        if (targetEl)
            settingsSectionObserver?.observe(targetEl);
    });
...
function applyUiLanguage() {
    ...
    if (settingsNavAppearanceButton)
        settingsNavAppearanceButton.textContent = t('heading.appearance');
    appearanceSubtitleEl.textContent = t('heading.appearanceSubtitle');
    runtimeSummaryEl.textContent = t('heading.runtime');
    if (settingsNavRuntimeButton)
        settingsNavRuntimeButton.textContent = t('heading.runtime');
    ...
    if (settingsNavLanguageAidsButton)
        settingsNavLanguageAidsButton.textContent = t('heading.languageAids');
    ...
    Array.from(sttProviderSelect.options).forEach((option) => {
        option.textContent = t(`provider.${option.value}`);
    });
    Array.from(mainSttProviderSelect.options).forEach((option) => {
        option.textContent = t(`provider.${option.value}`);
    });
